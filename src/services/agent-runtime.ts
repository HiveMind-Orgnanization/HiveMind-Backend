  ...
  if (opts?.swarmArtifactStep) {
    ...
    if (JSON_ARTIFACT_SPECIALIZATIONS.has(agent.specialization)) {
      ...
      return [
        ...
        "Pin `vite` to `^5.4.21` in devDependencies (NOT `^6`, `^7`, or `latest`). Vite 6/7 ships the experimental `rolldown` bundler which crashes on agent-generated React code with no useful error. Use `@vitejs/plugin-react` `^4.3.6`. Never list `rolldown`, `rolldown-vite`, or `@rolldown/*` in deps.",
        "PostCSS and Tailwind config files: name them `postcss.config.cjs` and `tailwind.config.cjs` (NOT `.js` / `.ts`). Use `module.exports = { ... }` syntax. Avoid `export default { ... }` for these two files — Vite tries to load them as Node and the ESM/CJS mismatch makes the build fail with `postcss.config.js at load`.",
